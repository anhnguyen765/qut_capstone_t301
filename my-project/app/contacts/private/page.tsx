...
export default function PrivateContacts() {
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = "private_contacts.csv";
    a.click();
  ...
  useEffect(() => {
    fetch("/api/contacts")
      .then(res => res.json())
      .then((data: Contact[]) => {
        // Ensure data is always an array and filter by Private group
  ...
  const handleAddContact = async () => {
    try {
      ...
      setNewContact({ name: "", email: "", phone: "", group: "Private", notes: "", opt1: true, opt2: true, opt3: true });
      // Refresh contacts
        ...
        <main className="mx-auto mt-6">
          <ul className="divide-y divide-[var(--border)] bg-[var(--background)] rounded-lg shadow">
            {filteredContacts.length === 0 ? (
              <li className="p-6 text-center text-[var(--foreground)]">No private contacts found.</li>
            ) : (
          ...
          <div className="bg-card text-card-foreground rounded-lg shadow-lg p-8 w-full max-w-md max-h-[90vh] overflow-y-auto border border-border">
            ...
            >
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button type="submit">Add Private Contact</Button>
              </div>
            </form>
          </div>
        </div>
