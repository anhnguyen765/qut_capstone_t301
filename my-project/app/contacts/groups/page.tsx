...
export default function GroupsContacts() {
  ...
  // Fetch contacts from API and filter by Groups group
  useEffect(() => {
    fetch("/api/contacts")
      .then(res => res.json())
      .then((data: Contact[]) => {
        // Ensure data is always an array and filter by Groups group
        if (Array.isArray(data)) {
          setContacts(data.filter(contact => contact.group === "Groups"));
        } else {
  ...
  const handleAddContact = async () => {
    try {
      ...
      setNewContact({ name: "", email: "", phone: "", group: "Groups", notes: "", opt1: true, opt2: true, opt3: true });
      // Refresh contacts
        ...
        <main className="mx-auto mt-6">
          <ul className="divide-y divide-[var(--border)] bg-[var(--background)] rounded-lg shadow">
            {filteredContacts.length === 0 ? (
              <li className="p-6 text-center text-[var(--foreground)]">No group contacts found.</li>
            ) : (
      ...
      {showAddDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-card text-card-foreground rounded-lg shadow-lg p-8 w-full max-w-md max-h-[90vh] overflow-y-auto border border-border">
            <h2 className="text-xl font-bold mb-4 text-card-foreground">Add New Group Contact</h2>
            <form
              onSubmit={e => {
                e.preventDefault();
                handleAddContact();
            ...
            >
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button type="submit">Add Group Contact</Button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="bg-card text-card-foreground rounded-lg shadow-lg p-8 w-full max-w-md max-h-[90vh] overflow-y-auto border border-border">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-card-foreground">
                {isEditing ? "Edit Group Contact" : "Group Contact Details"}
              </h2>
              {!isEditing && (
                <Button
                  variant="outline"
